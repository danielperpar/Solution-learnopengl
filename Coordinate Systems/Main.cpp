...
int main()
{
	...
	unsigned char *data = stbi_load("textures/container.jpg", &width, &height, &nr_channels, 3);
	if (data)
	{
	...
	data = stbi_load("textures/awesomeface.png", &width, &height, &nr_channels, 3);
	if (data)
	{
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
		glGenerateMipmap(GL_TEXTURE_2D);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		shader.Use();
		shader.SetFloat("mixValue", mix_value);
		...
		for (int i = 0; i < 10; i++)
		{
			...
			float angle = (i % 3 == 0) ? (float)glfwGetTime() : i;
			model = glm::rotate(model, glm::radians(20.0f * angle), glm::vec3(0.5f, 1.0f, 0.0f));
...
void processInput(GLFWwindow *window)
{
	...
	{
		mix_value += 0.1;
		std::cout << "mix = " << mix_value << std::endl;
	}
