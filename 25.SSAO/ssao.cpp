...
int main()
{
	...
	Shader ssaoShader("resources/shaders/ssao.vs", "resources/shaders/ssao.fs");
	Shader ssaoblurShader("resources/shaders/ssaoblur.vs", "resources/shaders/ssaoblur.fs");
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, SCR_WIDTH, SCR_HEIGHT, 0, GL_RGB, GL_FLOAT, NULL);  //internal format must be GL_RGB16F not GL_RGB16 !!!!!
	...
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
		std::cout << "framebuffer ssaoFBO not complete" << std::endl;

	glBindFramebuffer(GL_FRAMEBUFFER, 0);

	// framebuffer for blur effect on ssao texture
	// ---------------------------------------
	unsigned int blurFBO;
	glGenFramebuffers(1, &blurFBO);
	glBindFramebuffer(GL_FRAMEBUFFER, blurFBO);
	unsigned int blurSSAOMap;
	glGenTextures(1, &blurSSAOMap);
	glBindTexture(GL_TEXTURE_2D, blurSSAOMap);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, SCR_WIDTH, SCR_HEIGHT, 0, GL_RGB, GL_FLOAT, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, blurSSAOMap, 0);
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
		std::cout << "framebuffer blurFFAO not complete" << std::endl;
	...
	{
		glm::vec3 sample(2.0f * randomFloats(generator) - 1.0f, 2.0f * randomFloats(generator) - 1.0f, randomFloats(generator));
	...
	lightingPassShader.SetVec3("lightColor", lightColor);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glBindFramebuffer(GL_FRAMEBUFFER, gBuffer);
			...
			geometryPassShader.SetInt("flipNormal", 1);			
			renderCube();
			geometryPassShader.SetInt("flipNormal", 0);
		...
		glBindFramebuffer(GL_FRAMEBUFFER, ssaoFBO);
			...
			ssaoShader.Use();
			for (int i = 0; i < 64; i++)
				ssaoShader.SetVec3("samplesKernel[" + std::to_string(i) + "]", ssaoKernel[i]);
		...
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		lightingPassShader.Use();
		glm::vec3 lightPosViewSpace = glm::vec3(view * glm::vec4(lightPosition, 1.0f));
		lightingPassShader.SetVec3("lightPos", lightPosViewSpace);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, gPosition);
